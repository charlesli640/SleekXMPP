...
class MUCBot(sleekxmpp.ClientXMPP):
    ...
    def start(self, event):
        ...
        query = ET.Element('{http://jabber.org/protocol/muc#admin}query')
        x = ET.Element('item', affiliation='member')
        query.append(x)
        iq = self.make_iq(id="324098foifa37880", ifrom=self.boundjid.full, ito=self.room, itype='get')
        iq.append(query)
        try:
            iq.send(timeout=60)
        except IqError as e:
...
if __name__ == '__main__':
    ...
    if opts.jid is None:
        opts.jid = "admin@jitsi-meet-charles.magnet.com" #raw_input("Username: ")
    if opts.password is None:
        opts.password = "Magnet123" # getpass.getpass("Password: ")
    if opts.room is None:
        opts.room = raw_input("MUC room: ")
    if opts.nick is None:
        opts.nick = "admin"  #raw_input("MUC nickname: ")
